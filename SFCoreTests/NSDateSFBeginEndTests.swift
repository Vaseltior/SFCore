...
class NSDateSFBeginEndTests: XCTestCase {
  ...
  func testSFBeginningOfDay() {
    ...
    let d = Date(timeIntervalSince1970: 1454907600)
  ...
  func testYearInSeconds() {
    XCTAssertEqual(Date.sfYearInSeconds(), 31536000)
  }

  func testNumberOfDaysBetweenDates() {
    let d1 = Date(timeIntervalSince1970: 0)
    var d2 = Date(timeIntervalSince1970: 8600)
    XCTAssertEqual(Date.sfNumberOfDaysBetween(d1, endDate: d2), 0)
    d2 = Date(timeIntervalSince1970: 86400)
    XCTAssertEqual(Date.sfNumberOfDaysBetween(d1, endDate: d2), 1)
  }
  ...
  func testComparisons() {
    ...
    XCTAssertTrue(d1 < d2)
    XCTAssertTrue(d2 > d1)
    d2 = Date(timeIntervalSince1970: 0)
    XCTAssertTrue(d1 == d2)
    XCTAssertTrue(d1 <= d2)
  }
}
